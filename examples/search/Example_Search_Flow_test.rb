...
class Example_Search_TestClass < Test::Unit::TestCase
  ...
  def test_Search_003_FLOW
    ...
    begin
      ...
      # Find the Row of data matching the Execution Environment (The Row data is in a HASH)
      $aSpreadsheet_URL.each do | hRow_ofData |
        # Find a row of data who's ENV Column setting matches the current Environment
        # and who's ROLE column setting matches the current Role
        if(hRow_ofData["ENV"] == $sExecution_Env)
          # Get the setting in that row from the proper Column
          $sSearchFieldName = hRow_ofData["SearchFieldName"]
          $sSubmitButtonName = hRow_ofData["SubmitButtonName"]
        end

      end # Use the Environment setting to name of the query fields for the current environment

      puts2("field name: #{$sSearchFieldName}")
      puts2("button name: #{$sSubmitButtonName}")

      hSearchTerm = []
      hExpectedLinkHref = []
      hExpectedText = []

      # Find the search terms and expected results using the Environment settings
      # Find the Row of data matching the Execution Environment (The Row data is in a HASH)
      $aSpreadsheet_TERMS.each do | hRow_ofData |
        ...
        if(hRow_ofData["ENV"] == $sExecution_Env)
          ...
          # Get the setting in that row from the proper Column
          sSearchTerm = hRow_ofData["SearchTerm"].to_s
          sExpectedLinkHref = hRow_ofData["ExpectedLinkHref"].to_s
          sExpectedText = hRow_ofData["ExpectedText"].to_s

          hSearchTerm  << sSearchTerm.to_s
          hExpectedLinkHref << sExpectedLinkHref.to_s
          hExpectedText << sExpectedText.to_s

        end

      end # Find the search terms and expected results

      #browser.show_html_tag_attributes(["button", "form", "text_field"])

      hSearchTerm.each_with_index do | sTerm, iIndex |

        puts2("\nSearch number: " + (iIndex + 1).to_s)

        sExpectedLinkHref = hExpectedLinkHref[iIndex]
        sExpectedText = hExpectedText[iIndex]

        # Enter the query string
        puts2("Entering qwery: #{sTerm}")
        browser.text_field(:name, "#{$sSearchFieldName}").set "#{sTerm}"
        ...
        puts2("Validating expected text: #{sExpectedText}")
        assert(browser.text =~/#{$sExpectedText}/)
  ...
  end # END TestCase - test_Search_003_FLOW
